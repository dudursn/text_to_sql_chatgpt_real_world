...
def get_sqls_self_consistency(results, select_number, db):
    ...
    return chosen_p_sqls[0]


def exec_on_db(query: str, db):
    try:
        result = db.run(query)
...
def get_exec_output(
    ...
    flag, sql_denotation = exec_on_db(sql, db)
